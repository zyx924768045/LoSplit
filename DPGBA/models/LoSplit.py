...
class LoSplit(nn.Module):
    ...
    def split_node(self, idx_train, labels, num_epoch, num_attach):
        ...
        # print(f"FPR: {fpr:.4f}")
        # idx_filtered = idx_train[idx_losses_record]  # labels_list = labels[idx_filtered]
        ...
        #         plt.hist(losses_clean_np[cls_mask], bins=30, alpha=0.75,
        #                 color=cmap(i % 10), edgecolor='black',
        #                 linewidth=hist_linewidth, 
        #                 label=f'Clean (Class {self.args.target_class})')

        # plt.hist(losses_poison_np, bins=25, alpha=0.75, color='crimson',
        ...
        #         frameon=False, 
        #         prop={'weight': 'bold'},
        #         handlelength=1.25,             # 控制图例线条长度
        #         handletextpad=0.3,            # 控制图例符号和文字间距
        #         markerscale=0.6               # 缩小图例标记（点/柱子）的大小
        #     )

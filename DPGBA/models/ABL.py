...
class ABL(nn.Module):
    ...
    def forward_sur(self, features, edge_index, edge_weight):
        if(self.model_name == 'GCN'):
            output, x = self.sur_model.forward(features, edge_index, edge_weight)
        elif(self.model_name in ['GAT','GraphSAGE','GIN']):
    ...
    def _train_with_val(self, labels, idx_train, idx_val, train_iters, verbose, num_attach):
        ...
        for i in range(train_iters):
            ...
            if(i < isolate_epoch):
                ...
                # loss_attach = 0*loss_attach_decouple + 1*loss_attach_unlearn

                # loss_train = torch.cat([loss_clean, loss_attach])
                # # loss_train = loss_clean + loss_attach
                # loss_train = torch.mean(loss_train)
                # loss_train.backward()
                # optimizer.step()
                ...
                # loss_attach_decouple =  F.nll_loss(output[idx_isolated], reshuffle_labels[idx_isolated], reduction='none')
                # # loss_clean = F.nll_loss(output[idx_clean], labels[idx_clean], reduction='none')
                # loss_attach = 0.3*loss_attach_decouple + 0.7 * loss_attach_unlearn

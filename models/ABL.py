...
class ABL(nn.Module):
    ...
    def fit(self, features, edge_index, edge_weight, labels, idx_train, idx_val=None, train_iters=200, verbose=False, finetune1=False, finetune2=False, attach=None, clean=None, target_label=0, num_attach=160):
    ...
    def finetune2(self, labels, idx_train, idx_val, idx_attach, idx_clean, train_iters, verbose, target_label):
        ...
        for i in range(train_iters):
            ...
            loss_attach = 0.8*loss_attach_decouple + 0.2*loss_attach_unlearn
    ...
    def _train_with_val(self, labels, idx_train, idx_val, train_iters, verbose, num_attach):
        ...
        for i in range(train_iters):
            ...
            if(i < isolate_epoch):
                ...
                # probs = F.softmax(output[idx_isolated], dim=1)
                # target_probs = probs[range(len(labels[idx_isolated])), labels[idx_isolated]]
                # loss_train_2 = torch.mean(target_probs)  # Mean of probabilities of correct labels
                ...
                loss_train_isolated = torch.relu(loss_train_isolated)
                ...
                # mask = torch.zeros_like(labels, dtype=torch.bool)
                # mask[idx_isolated] = 1  
                # choices = torch.tensor([i for i in range(num_classes) if i != 0])
                # random_replacements = choices[torch.randint(0, len(choices), size=(mask.sum(),))]
                # reshuffle_labels = labels
                # reshuffle_labels[mask] = random_replacements        
                ...
                # optimizer.zero_grad()
                # output = self.forward(self.features, self.edge_index, self.edge_weight)
                # loss_attach_unlearn = -1 * F.nll_loss(output[idx_isolated], labels[idx_isolated], reduction='none')
                # loss_attach_unlearn = torch.relu(loss_attach_unlearn)
                # loss_attach_decouple =  F.nll_loss(output[idx_isolated], reshuffle_labels[idx_isolated], reduction='none')
                # # loss_clean = F.nll_loss(output[idx_clean], labels[idx_clean], reduction='none')
                # loss_attach = 0.7*loss_attach_decouple + 0.3 * loss_attach_unlearn
                # loss_attach = torch.mean(loss_attach)
